...
3. For each hash, fetch the receipt independently of Dune with viem (already a dependency) against a public RPC such as `https://eth.drpc.org` (publicnode and cloudflare-eth return null receipts for older txs). Decode the `Subscription` / `Redemption` log with the ABI from `docs/CONTRACT_RESEARCH.md`, and confirm the emitting address is the expected InstantManager.
